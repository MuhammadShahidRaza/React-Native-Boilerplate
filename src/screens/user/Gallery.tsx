...
export const Gallery = ({ data, itemData }: { data: any; itemData: any }) => {
  ...
  const fetchGallery = useCallback(
    async (page: number) => {
      if (isLoadingRef.current || !data?.id || !itemData?.id) return;
      try {
  ...
  useEffect(() => {
    if (!didLoad && (data?.id || itemData?.id)) {
      fetchGallery(1);
      setGalleryListPage(1);
    }
  }, [didLoad, data?.id, itemData?.id, fetchGallery]);
  ...
  return (
    <View style={styles.tabContent}>
      <FlatListComponent
        ...
        onRefresh={onRefresh}
        onEndReached={() => {
          if (!isLoadingRef.current && hasMore && galleryListPage > 0) {
