          ...
          onPress={() => {
            onPressViewAll
              ...
              : navigate(SCREENS.VIEW_ALL, {
                  data: { items, headerTitle: itemHeading },
                });
          }}
          style={{
            color: COLORS.BORDER,
...
export type ItemCardData = {
  id: number;
  title: string;
  categoryName: CategoryNameTypes;
...
// export const mapToItemCardData = ({
//   data,
//   isItemFlow,
// }: {
//   data: CategoryItem | Vendor;
//   isItemFlow: boolean;
// }): ItemCardData => {
//   const categoryName = isItemFlow
//     ? (data as CategoryItem)?.itemCategory?.category?.title
//     : (data as Vendor)?.category?.title;

//   return {
//     id: data?.id,
//     title: (data as Vendor)?.business_name ?? (data as CategoryItem)?.title,
//     categoryName,
//     ratingAvg: safeString(String(data?.rating_avg ?? '0.0')),
//     isLiked: data?.is_liked,
//     city: (data as Vendor)?.city ?? (data as CategoryItem)?.eventDetail?.city,
//     country: (data as Vendor)?.country ?? (data as CategoryItem)?.eventDetail?.country,
//     distance: data?.distance,
//     date: (data as CategoryItem)?.eventDetail?.date,
//     startTime: (data as CategoryItem)?.eventDetail?.start_time,
//     endTime: (data as CategoryItem)?.eventDetail?.end_time,
//     imageUrl: (data as Vendor)?.business_logo ?? (data as CategoryItem)?.media?.[0]?.media_url,
//     categoryId: (data as Vendor)?.category?.id ?? (data as CategoryItem)?.category_id,
//     ...(isItemFlow ? { vendor: data } : { itemData: data }),
//   };
// };
...
export const mapToItemCardData = ({
  ...
  if (isItemFlow) {
    ...
    return {
      id: vendor.id,
      title: vendor.business_name ?? vendor?.full_name,
      categoryName: vendor.category?.title as CategoryNameTypes,
      ratingAvg: safeString(String(vendor.rating_avg ?? '0.0')),
      isLiked: vendor.is_liked,
      city: vendor.city,
      ...
      startTime: undefined,
      endTime: undefined,
      imageUrl: vendor.business_logo ?? vendor?.profile_image ?? IMAGES.USER,
      categoryId: vendor.category?.id ?? 0,
      vendor,
    };
  }
};
